...
1. Нажатие на Enter создаёт пустую строку. Так можно визуально в консоли разделять инфу
2. Описать, как отключать доступ по паролю
3. Как настраивать SSH
...
4. Как можно поймать майнер
5. Как узнать, какие ещё есть доступные аргументы для команд
    - `--help`, `-h`
...
20. `date`
    - Проверка времени 
22. Как доп. страховка на будущее — при экспериментах с маршрутизацией на удалённом сервере полезно запускать команду через screen/tmux с отложенным автоматическим "откатом" (например, sudo timeout 60 wg-quick up wg0 в отдельной сессии — если сломается, само отключится через 60 секунд, и вы не потеряете доступ).
23. 
...
## Работа с файлами
1. Передать с одного компа на другой внутри локальной сети
    - `scp /путь/к/файлу.txt username@192.168.1.100:/путь/на/сервере/`
    - **Можно сразу и переименовать**
        - `scp /путь/к/файлу.txt username@192.168.1.100:/путь/на/сервере/новое_имя.txt` 
2. Переименовать
    - `mv старое_имя.txt новое_имя.txt`
3. 

## Создание папок и выдача корректных прав

Чтобы не возникали ошибки доступов
1. `id $USER`
    - Узнаём свой UID и GID
2. `mkdir -p ~/<название папки>`
3. `sudo chown -R 1000:1000 ~/<название папки>`
    - Выдача прав. 1000 - это UID и GID
...
# Работа с консолью
Привет! С удовольствием подскажу. В баше для этого есть очень удобные горячие клавиши, которые работают по умолчанию.

Когда ты листал историю (например, стрелочками вверх/вниз) и нашел нужную команду, переместиться в самое её начало можно с помощью комбинации:

Ctrl + A

Другие полезные шорткаты для навигации:
Если тебе нужно не просто в начало, а чуть точнее, вот три главные комбинации, которые экономят кучу времени:

Ctrl + E — переместиться в самый конец строки (противоположность Ctrl + A).

Alt + B — переместиться на одно слово назад (влево).

Alt + F — переместиться на одно слово вперёд (вправо).

💡 Маленький нюанс: Если ты работаешь на macOS в стандартном терминале, клавиша Alt (Option) по умолчанию может вводить спецсимволы вместо отправки команд башу. Чтобы это исправить, зайди в Настройки терминала → Профили → Клавиатура и поставь галочку "Использовать Option как мета-клавишу" (Use Option as Meta key).
